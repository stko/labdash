...
class SplPlugin(SplThread):
    ...
    def __init__(self, modref):
            ...
            "config.json",
            {
                ...
                "actual_settings": {
                    ...
                    "www_root_dir": os.path.realpath(
                        os.path.join(self.program_dir, "../../../web/")
                    ),
                    "epa_root_dir": [
                        os.path.realpath(
                            os.path.join(self.program_dir, "../../../web/examples/")
                        )
                    ],
                    "eol_root_dir": [
                        os.path.realpath(
                            os.path.join(self.program_dir, "../../../web/eol/")
                        )
                    ],
                },
            },
        ...
        def handle_eol(path):
            ...
            if not elements:  # empty path
                return self.app.response_class(
                    response="<h2>No EOL reference in URL</h2>", status=404
                )
            if not elements[0] in self.eol_directory:
                return self.app.response_class(
                    response="<h2>Unknown EOL reference in URL</h2>", status=404
                )
            ...
            if len(elements) == 1:  # this is a request to load a new EOL
                if "html" in eol_info:  # does this package has its own main html page?
                    ...
                    return send_from_directory(
                        os.path.join(
                            self.config.read("actual_settings")["www_root_dir"],
                            "theme",
                            self.theme,
                        ),
                        "startpage_eol.html",
        ...
        def send_libs(path):
            return send_from_directory(
                os.path.join(
                    self.config.read("actual_settings")["www_root_dir"], "libs"
                ),
                path,
            )
        ...
        def handle_epa(path):
            ...
            if not elements:  # empty path
                return self.app.response_class(
                    response="<h2>No EPA reference in URL</h2>", status=404
                )
            if not elements[0] in self.epa_directory:
                return self.app.response_class(
                    response="<h2>Unknown EPA reference in URL</h2>", status=404
                )
            self.actual_file_id = elements[0]
        ...
        def send_theme(theme, path):
            ...
            return send_from_directory(
                os.path.join(
                    self.config.read("actual_settings")["www_root_dir"], "theme", theme
                ),
                path,
            )
    ...
    def event_listener(self, queue_event):
        ...
        if queue_event.type == defaults.EPA_CATALOG:
            ...
            if (
                self.epa_catalog_xml_string
                and self.eol_catalog_xml_string
                and self.awaiting_initial_content_list
            ):
